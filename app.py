...
# --- FIX VISIBILITY & STYLING ---
st.markdown("""
    <style>
    /* Force dark text for readability on light backgrounds */
    html, body, [data-testid="stWidgetLabel"], .stTabs button p {
        color: #1c3d5a !important;
    }
    
    /* Style the tabs so they are visible */
    .stTabs button {
...
if not st.session_state.on_shift:
    ...
    selected_user = st.selectbox("Choose your identity:", engineers)
...
# --- VIEW 2: LOGGED IN DASHBOARD ---
    ...
    with st.sidebar:
        st.title("🛠️ NotEMMA")
        st.write(f"Active: **{st.session_state.current_user}**")
        if st.button("🚪 Logout / Finish"):
            st.session_state.on_shift = False
            st.rerun()
        st.divider()
    ...
    with tab1:
        ...
        task = st.selectbox("Task", ["DRUPS testing", "Flushing", "Fire Door Inspection", "Change light", "Change lock"])
    ...
    with tab3:
        ...
        conn.close()
        for l in logs:
            st.markdown(f"""<div class="handover-card"><div class="engineer-tag">👷 {l[0]} | {l[2]}</div>{l[1]}</div>""", unsafe_allow_html=True)

    with tab4:
        st.subheader("Recent Activity")
        conn = sqlite3.connect(DB_FILE)
        history = conn.execute("SELECT engineer, job, timestamp FROM history ORDER BY id DESC LIMIT 10").fetchall()
        conn.close()
        st.table(history)